...
install_deps() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        if ! "$PIP_PYTHON" -m pip install -e '.[termux-all]' -c constraints-termux.txt -i https://mirrors.aliyun.com/pypi/simple ; then
            log_warn "Termux broad profile (.[termux-all]) failed, trying baseline Termux profile..."
            if ! "$PIP_PYTHON" -m pip install -e '.[termux]' -c constraints-termux.txt -i https://mirrors.aliyun.com/pypi/simple ; then
                log_warn "Termux baseline profile (.[termux]) failed, trying base install..."
                if ! "$PIP_PYTHON" -m pip install -e '.' -c constraints-termux.txt -i https://mirrors.aliyun.com/pypi/simple ; then
